...
- ``flaresolverr`` — fetch through a FlareSolverr proxy, which drives a real
  ...
  HTML. Point it at your instance with ``PYJAZZMUSICARCHIVES_FLARESOLVERR_URL``
...
def flaresolverr_endpoint() -> str:
    """Return the configured FlareSolverr base URL (``PYJAZZMUSICARCHIVES_FLARESOLVERR_URL``)."""
    return os.environ.get("PYJAZZMUSICARCHIVES_FLARESOLVERR_URL", "").strip()
...
def flaresolverr_html(url: str, endpoint: Optional[str] = None,
    ...
    fallback. Point it at your instance with
    ``PYJAZZMUSICARCHIVES_FLARESOLVERR_URL`` (e.g. ``http://192.168.1.116:8191``).
    """
    import requests
    endpoint = (endpoint or flaresolverr_endpoint() or "http://localhost:8191").rstrip("/")
    timeout_ms = timeout_ms or int(os.environ.get("PYJAZZMUSICARCHIVES_FLARESOLVERR_TIMEOUT", "60000"))
    resp = requests.post(
        f"{endpoint}/v1",
...
def get_html(path: str, **params: Any) -> str:
    ...
    Transport is chosen by ``PYJAZZMUSICARCHIVES_TRANSPORT`` (``requests`` /
    ``curl_cffi`` / ``wayback`` / ``flaresolverr``). If the variable is unset
    but ``PYJAZZMUSICARCHIVES_FLARESOLVERR_URL`` is set, FlareSolverr is used
    automatically. ``PYJAZZMUSICARCHIVES_WAYBACK_FALLBACK=1`` adds a Wayback
    fallback on any live failure. See the module docstring.
