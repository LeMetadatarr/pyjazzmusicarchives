...
jazzmusicarchives.com sits behind a Cloudflare challenge. Transport is
configurable two ways — **constructor kwargs** on :class:`Transport` (or the
high-level :class:`pyjazzmusicarchives.JazzMusicArchives` client), or
**environment variables** as fallback defaults. Explicit kwargs always win.
...
Environment fallbacks: ``PYJAZZMUSICARCHIVES_TRANSPORT``,
``PYJAZZMUSICARCHIVES_FLARESOLVERR_URL``,
``PYJAZZMUSICARCHIVES_FLARESOLVERR_TIMEOUT`` (ms),
``PYJAZZMUSICARCHIVES_WAYBACK_FALLBACK``.
...
class Transport:
    ...
    def _fs_url(self) -> str:
        return (self.flaresolverr_url
                or os.environ.get("PYJAZZMUSICARCHIVES_FLARESOLVERR_URL", "").strip())

    def _fs_timeout(self) -> int:
        if self.flaresolverr_timeout_ms is not None:
            return self.flaresolverr_timeout_ms
        return int(os.environ.get("PYJAZZMUSICARCHIVES_FLARESOLVERR_TIMEOUT", "60000"))

    def _resolved_mode(self) -> str:
        if self.mode:
            return self.mode
        env = os.environ.get("PYJAZZMUSICARCHIVES_TRANSPORT", "").strip().lower()
        if env:
            return env
        if self._fs_url():
            return "flaresolverr"
    ...
    def _wayback_fallback(self) -> bool:
        ...
        return _truthy(os.environ.get("PYJAZZMUSICARCHIVES_WAYBACK_FALLBACK"))
