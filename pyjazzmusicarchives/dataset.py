"""Flat, tabular rows from jazzmusicarchives data for Hugging Face datasets.

Two join-able streams sharing ``artist_slug``:

- :func:`artist_rows` — one row per artist (the headline table)
- :func:`album_rows`  — one row per album, with per-album subgenre and ratings
...
from pyjazzmusicarchives.artists import fetch_artist, iter_artists
...
def artist_detail_row(detail: ArtistDetail) -> Dict[str, Any]:
    """Yield one rich row for a fetched :class:`ArtistDetail` (styles + bio)."""
    return {
        "artist_slug": detail.slug,
        "name": detail.name,
...
def album_rows(detail: ArtistDetail) -> Iterator[Dict[str, Any]]:
    """Yield one row per album on an artist page."""
...
def build_dataset(
    ...
    *,
    ...
    ``with_detail=True`` fetches each artist page and emits the richer
    :func:`artist_detail_row` (styles + bio) — one HTTP request per artist.
    """
    if with_detail:
        for a in iter_artists(letters):
            yield artist_detail_row(fetch_artist(a.slug))
    else:
