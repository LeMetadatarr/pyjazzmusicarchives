...
def _clear_env(monkeypatch):
    for k in ("PYJAZZMUSICARCHIVES_TRANSPORT", "PYJAZZMUSICARCHIVES_FLARESOLVERR_URL",
...
def test_transport_kwarg_beats_env(monkeypatch):
    monkeypatch.setenv("PYJAZZMUSICARCHIVES_TRANSPORT", "requests")
...
def test_client_kwargs(monkeypatch):
    ...
    assert jma.JazzMusicArchives(wayback=True).transport._resolved_mode() == "wayback"
    c = jma.JazzMusicArchives(flaresolverr_url="http://192.168.1.116:8191")
